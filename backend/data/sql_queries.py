...
class MainQueries:
    ...
    DEPT_HOSP = f"""
        ...
        JOIN mm.profile_med pm ON pm.id = d.profile_med_    
        WHERE ec.create_dt BETWEEN CURRENT_DATE - INTERVAL '18 hours' AND CURRENT_DATE + INTERVAL '6 hou    
        GROUP BY  pm.id,pm.name
        ORDER BY pm.id ASC;
    """
    ...
    OAR_CURRENT_QUERY = f"""
        ...
        FROM mm.mdoc m
        JOIN mm.hospdoc h ON h.mdoc_id = m.id
        JOIN mm.people p ON p.id = m.peopl  
    ...
    def create_dmk_query(self) -> list[str]:
        """
        Create list of queries from class attributes needed for data to DMK DB.

        :return: List of single queries.
        """
        result = [self.ARRIVED, self.SIGNOUT, self.OAR_ARRIVED_QUERY, self.DEPT_HOSP]
        return result

    def create_kis_query(self) -> list[str]:
        """
        Create list of queries from class attributes needed for data to front-end.

        :return: List of lists.
        """
        dmk_queries = self.create_dmk_query()[:-1]
    ...
    def chosen_date_query(queryset: Union[str, list], chosen_date: str) -> list:
        ...
        :param queryset: Original class attribute query contains today date filter.
        :param chosen_date: Date for filtering that was chose users.
        :return: Changed query contains actual chosen date.
        """
